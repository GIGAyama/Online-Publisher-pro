 ...
 *   2,000 行の JSX を「開くたびに」ブラウザの中で翻訳していた。
...
const VENDOR = [
  ...
  // 児童に配る URL の QR。外部の QR 生成サービスへ URL を送らないため、画面の中で作る。
  // UMD なので、素の <script> で読むと window.QRCode に入る（画面はそれを使う）。
  ['qrcode', 'node_modules/qrcodejs2/qrcode.js'],
];
...
/* 原稿の差分表示（diff_match_patch）。
   ...
   そのまま <script> に入れると module が無くて落ちるので、受け皿を用意して
   画面が使う名前へ移す。 */
{
  const src = readFileSync(join(ROOT, 'node_modules/diff-match-patch/index.js'), 'utf8');
  vendor +=
...
const tmp = mkdtempSync(join(tmpdir(), 'op-build-'));
