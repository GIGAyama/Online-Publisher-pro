...
> **✅ このロールアウトは完了しました。** 改修後の状態は `npm run check` で
> いつでも確かめられます（56項目すべて合格）。
> 下の表は「何がどう悪かったか」の記録として、**改修前のまま**残してあります。
>
> **2026-08-13追補 — マルチテナント化**：当時未対応だったB3/B5について、
> `appsscript.json` の追加、アクセスユーザー実行、教師別User Properties、学級コード、
> Googleアカウントのドメイン照合、署名付きセッション、作者キーによる児童本人データの分離を実装した。個人用Googleアカウントではリンク共有、Workspaceでは原則ドメイン内共有を使用する。
> 以下のB3/B5記録と「今回直さないもの」節は2026-08-08時点の監査履歴である。
>
> | フェーズ | 内容 | 状態 |
> |---|---|:--:|
> | P0 | LICENSE / .gitignore / dependabot / 本監査書 | ✅ |
> | P1 | 表示（dvh・safe-area・clamp・44px・reduced-motion・印刷）と PWA 一式 | ✅ |
> | P1(a11y) | 文字コントラスト（配色に触れるため独立コミット） | ✅ |
> | P2 | アイコンPNGの再圧縮（画素の変化なし） | ✅ |
> | P3 | MANUAL に PWA導入・印刷確認手順、README に制限とクォータ | ✅ |
> | P4 | 品質ゲート `npm run check` と CI | ✅ |
> | — | CDN依存の解消・CSP・OAuthスコープ・提示モード | ⏸ 未対応（下記「今回直さないもの」参照） |
>
> **先生にお願いしたいこと（2つだけ）**
> 1. `pwa/` を GitHub Pages で公開している場合、児童の端末で一度開き直してください。
>    新しい Service Worker に入れ替わります。以後は更新があると
>    「あたらしい バージョンが あります」と画面に出ます。
> 2. はじめて印刷する前に、[MANUAL.md §7-5](MANUAL.md#7-5-印刷の前に確認すること) の
>    4項目をプレビューで確認してください（特に「背景のグラフィック」のチェック）。
...
| D6 | 320px 幅で横スクロールが出ない | ✅ | `google.script.run` を差し替えたローカル版を Chromium で実際に描画して実測。320 / 375 / 810 / 1366px のいずれでも横スクロールなし（`min-w-0` / `truncate` / `shrink-0` が効いている） | 念のため `overflow-x` ガードを追加 |
